...
class B_Model(BaseModel):
    ...
    def optimize_parameters(self, step):
        ...
        # ipdb.set_trace()
        srs, kernels = self.netG(self.var_L)
        ...
        for ind in range(len(srs)):
            ...
            # ipdb.set_trace()
            if(ind == 0):
                d_kr, k_correct = self.cri_kernel(kernels[ind], self.lr_blured, self.lr)
            else:
                d_kr = self.cri_pix(kernels[ind], k_correct)
